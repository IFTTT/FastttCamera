...
- (void)testCropPerformance {
    [self measureBlock:^{
        ...
        for (NSInteger i=0; i<10000; i++) {
            __unused UIImage *croppedImage = [self.image fastttCroppedToOutputRect:outputRect];
        }
    }];
}
...
- (void)testRotatedPreviewPerformance {
    [self measureBlock:^{
        for (NSInteger i=0; i<100000; i++) {
            __unused UIImage *rotatedImage = [self.image fastttRotatedToMatchCameraView];
        }
    }];
}
...
- (void)testScaleToSizePerformance {
    [self measureBlock:^{
        __unused UIImage *scaledImage = [self.image fastttScaledToSize:CGSizeMake(600.f, 500.f)];
    }];
}

- (void)testScaleToMaxDimensionPerformance {
    [self measureBlock:^{
        __unused UIImage *scaledImage = [self.image fastttScaledToMaxDimension:600.f];
    }];
}

- (void)testScaleToScalePerformance {
    [self measureBlock:^{
        __unused UIImage *scaledImage = [self.image fastttScaledToScale:0.3f];
    }];
}

- (void)testNormalizePerformance {
    [self measureBlock:^{
        __unused UIImage *normalizedImage = [self.image fastttNormalizeOrientation];
    }];
}
